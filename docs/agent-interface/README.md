...
The validator also checks the `inspect`/`changes`/`commit`/`publish`/`ci_*` operation-specific
...
and unrelated-state preservation, exact remote revision, verified irreversible
steps, exact CI run revisions, terminal conclusions, and wait-budget evidence. The
recorder runs that same machine-fixture
