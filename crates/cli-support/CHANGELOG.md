...
- Apply single-value URL redaction to every `scheme://` value embedded after a
  field/flag prefix or adjacent in JSON/list text while preserving only the
  conventional non-secret `ssh://git@host` transport identity.
