...
/// The oldest `gh` that understands `gh auth token --user <login>` — **2.40.0**,
/// the release that introduced multi-account support (`gh auth switch`, per-account
/// `auth status`). Verified against gh's own sources: `--user` is absent from
/// `auth token` in 2.39.x and present from 2.40.0 on. [`GhAccountToken`] turns the
/// `unknown flag` a pre-2.40 binary emits into an error naming this floor, instead
/// of surfacing a bare non-zero exit.
const MIN_MULTI_ACCOUNT: GitHubVersion = GitHubVersion {
    major: 2,
    minor: 40,
    patch: 0,
};

/// The environment variables `gh` reads a token from, removed from this crate's
/// own `gh auth token` probe. Otherwise `gh` could answer with the token sitting
/// in the *process* environment instead of the requested account's — the toolkit
/// would then act under a different identity than the caller asked for, silently.
/// (`gh` resolves a `--user` request from its config/keyring, not from the
/// environment; scrubbing keeps that true no matter which resolution path a future
/// `gh` takes.)
...
/// A cache slot: `OnceCell` so concurrent requests for the same `(login, host)`
/// resolve the token **once** (the losers await the winner's result) rather than
/// racing two `gh` spawns; `Arc` so the slot outlives the map lock, which is
/// released before the spawn.
type TokenCell = Arc<tokio::sync::OnceCell<Secret>>;

/// A [`CredentialProvider`] that supplies the token of **one specific `gh`
/// account**, resolved lazily with `gh auth token --user <login>`.
///
/// A machine can hold several `gh` logins for the same host (a personal and a work
/// account), but exactly one is *active*. When the active account can't see the
/// repository you target, every forge call fails, and the only ambient fix —
/// `gh auth switch` — rewrites the user's global gh state. This provider avoids
/// that: it asks `gh` for the token of the account you name and injects it per
/// operation, leaving the active account untouched. Attach it like any other
/// provider:
///
/// ```no_run
/// use std::sync::Arc;
/// use vcs_github::{GhAccountToken, GitHub, GitHubHost};
/// let gh = GitHub::new()
///     .with_host(GitHubHost::github_com())
///     .with_credentials(Arc::new(GhAccountToken::new("octocat")));
/// ```
///
/// The token is yielded as a plain token credential ([`Credential::token`]) — the
/// same shape [`GitHub::with_token`] and [`GitHub::with_env_token`] produce, so it
/// lands in the env var `gh` reads for the bound host (`GH_TOKEN`, or
/// `GH_ENTERPRISE_TOKEN` after [`GitHub::with_host`] bound a GHES host) and never
/// in `argv`.
///
/// **Fail-closed.** Naming a login is an explicit statement of *which identity* an
/// operation runs under, so a login whose token cannot be resolved is an **error**
/// naming the login and the host — never `Ok(None)`. Deferring to ambient auth
/// there would run the operation as the *active* account instead: a silent identity
/// swap, the exact failure this type exists to prevent.
///
/// **Isolation.** The `gh auth token` probe runs on this provider's own client, so
/// it never goes through the calling client's credential injection (which would
/// recurse straight back into this provider), and it runs with `GH_TOKEN`,
/// `GITHUB_TOKEN`, `GH_ENTERPRISE_TOKEN`, and `GITHUB_ENTERPRISE_TOKEN` **removed**
/// from its environment, so an ambient token in the process environment can't be
/// echoed back in place of the requested account's.
///
...
/// The consequence is deliberate and worth knowing: **a token rotated in the
/// external `gh` mid-session is not picked up** — build a new provider (or a new
...
/// - `gh auth token` fails (e.g. `no oauth token found for github.com account
///   octocat`) — an [`ErrorReason::Exit`] naming the login and host;
/// - `gh` succeeds but prints nothing usable — an [`ErrorReason::Parse`].
pub struct GhAccountToken<R: ProcessRunner = processkit::JobRunner> {
    ...
    /// Resolved tokens, keyed by `(login, host)` — the host as it arrived on the
    /// [`CredentialRequest`] (`None` when the caller bound no host), so a token
    /// resolved for one host is never reused for another.
    cache: Mutex<HashMap<(String, Option<String>), TokenCell>>,
}

impl GhAccountToken<processkit::JobRunner> {
    /// A provider for the `gh` account `login`, probing the real `gh` binary.
    #[must_use]
    pub fn new(login: impl Into<String>) -> Self {
        Self::build(vcs_cli_support::ManagedClient::new(BINARY), login)
    }
}

impl<R: ProcessRunner> GhAccountToken<R> {
    /// A provider for `login` whose `gh auth token` probe runs on `runner` — inject
    /// a fake in tests, exactly as [`GitHub::with_runner`] does for the client.
    #[must_use]
    pub fn with_runner(runner: R, login: impl Into<String>) -> Self {
        Self::build(
            vcs_cli_support::ManagedClient::with_runner(BINARY, runner),
            login,
        )
    }
    ...
    /// here would otherwise stall the operation that asked for the token.
    #[must_use]
    pub fn default_timeout(mut self, timeout: Duration) -> Self {
        self.probe = self.probe.default_timeout(timeout);
        self
    }

    /// The `gh` account login this provider resolves tokens for.
    #[must_use]
    pub fn login(&self) -> &str {
        &self.login
    }

    fn build(probe: vcs_cli_support::ManagedClient<R>, login: impl Into<String>) -> Self {
        let probe = AMBIENT_TOKEN_VARS
            .iter()
    ...
    async fn token(&self, host: Option<&str>) -> Result<Secret> {
        // Validate before consulting the cache: a malformed login/host must be a
        // diagnosable error every time, never something a cache hit papers over.
        validate_login(&self.login)?;
        let host = host.map(probe_hostname).transpose()?;
        // Take the slot under the lock, then drop the guard: the resolution below
        // awaits a `gh` spawn, and a std `MutexGuard` must not be held across it.
        let cell = self.cell(host.as_deref());
        cell.get_or_try_init(|| self.resolve(host.as_deref()))
            .await
            .cloned()
    }

    /// The cache slot for `(login, host)`, created empty on first request.
    fn cell(&self, host: Option<&str>) -> TokenCell {
        let key = (self.login.clone(), host.map(str::to_string));
        // A poisoned slot map stays usable: the only mutation here is inserting an
        // empty slot, so a panicking caller can't have left it half-updated — and
        // failing credential resolution because an unrelated task panicked would be
        // a worse outcome than reusing the map.
        let mut cache = self.cache.lock().unwrap_or_else(PoisonError::into_inner);
        Arc::clone(cache.entry(key).or_default())
    }

    /// Run `gh auth token --user <login>` (plus `--hostname <host>` when the
    /// request carried a host) and return the token it printed.
    async fn resolve(&self, host: Option<&str>) -> Result<Secret> {
        // The secret never enters argv: `gh` prints it on stdout, so only the
        // login and the host are arguments — safe for `--log-commands`.
        let mut args = vec![
            "auth".to_string(),
            "token".to_string(),
            "--user".to_string(),
        ...
        // `output_string` captures a non-zero exit instead of raising it, so the
        // failure is classified here — a raised `Exit` would carry the captured
        // *stdout*, i.e. the token, into the error.
        let result = self.probe.output_string(args).await?;
        if result.timed_out() {
            // Preserve the timeout classification, but with an empty stdout so a
            // partially-printed token can't ride along in the error.
        ...
        if !result.is_success() {
            return Err(self.probe_failed(host, &result));
        }
        let token = result.stdout().trim();
        if token.is_empty() {
            return Err(Error::parse(
                BINARY,
                format!("{} printed no token", self.subject(host)),
            ));
        }
        if token.contains(['\r', '\n']) {
            // Not a token: `gh` prints exactly one line. Refuse it rather than
            // feeding a multi-line value to a line-based consumer (a git
            // credential helper), and never echo the value itself.
            return Err(Error::parse(
                BINARY,
                format!(
                    "{} printed a multi-line value, not a token",
                    self.subject(host)
                ),
            ));
        }
    ...
    /// What the failing request *was*, for an error message: the account and the
    /// host it was asked for. Both are identifiers, never secrets — and naming both
    /// is what makes a failed resolution actionable ("which identity, on which
    /// server?") instead of a bare non-zero exit.
    fn subject(&self, host: Option<&str>) -> String {
        let login = &self.login;
        match host {
            Some(host) => format!("`gh auth token` for account {login:?} on host {host:?}"),
            None => format!("`gh auth token` for account {login:?} on gh's default host"),
        }
    }

    /// Classify a failed `gh auth token` run: a pre-2.40 `gh` that doesn't know
    /// `--user` gets the version-floor error; anything else surfaces as an exit
    /// naming the login and host. Neither carries the captured stdout.
    fn probe_failed(&self, host: Option<&str>, result: &ProcessResult<String>) -> Error {
        let stderr = result.stderr();
        if lacks_user_flag(stderr) {
            return Error::spawn(
                BINARY,
                std::io::Error::new(
                    std::io::ErrorKind::Unsupported,
                    format!(
                        "`gh auth token --user` requires gh >= {MIN_MULTI_ACCOUNT} \
                         (multi-account support); the installed gh rejects the `--user` flag"
                    ),
                ),
            );
        }
        Error::exit(
            ...
            // Never the captured stdout: that is where the token would be.
            "",
            format!("{} failed: {}", self.subject(host), stderr_hint(stderr)),
        )
    }
}
...
impl<R: ProcessRunner> CredentialProvider for GhAccountToken<R> {
    async fn credential(&self, request: &CredentialRequest<'_>) -> Result<Option<Credential>> {
        ...
        Ok(Some(Credential::token(secret)))
    }
}

/// Validate a `gh` account login before it reaches `argv`. Rejects an empty value,
/// a leading `-` (which `gh` would read as a flag), and any control or whitespace
/// character — the latter keeps a login out of the "extra line in a line-based
/// protocol / log" class of problem. Deliberately *not* a GitHub-username charset
/// check: GHES accounts are not bound by github.com's rules.
fn validate_login(login: &str) -> Result<()> {
    let well_formed = !login.is_empty()
        && !login.starts_with('-')
        && !login.chars().any(|c| c.is_control() || c.is_whitespace());
    if !well_formed {
        return Err(Error::spawn(
            BINARY,
            std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!("gh account login {login:?}: not a valid account login"),
            ),
        ));
    }
    Ok(())
...
/// The `--hostname` to ask `gh` for, derived from the host on a
/// [`CredentialRequest`]. A forge request carries a canonical bare host (what
/// [`GitHub::with_host`] bound), but a **git HTTPS** request carries git's own
/// `host[:port]` verbatim (`vcs_cli_support::https_host` deliberately keeps the
/// port and case so git's byte-for-byte host match still works) — while `gh` names
/// hosts without a port. Strip it, then apply the crate's ordinary host validation,
/// so a legitimate non-default-port remote resolves instead of being refused, and
/// an authority `gh` could never name (an IPv6 literal, an empty or flag-like host)
/// is still an error rather than a guess.
fn probe_hostname(host: &str) -> Result<String> {
    let bare = strip_port(host).ok_or_else(|| {
        invalid_host_error(host, "no GitHub host could be determined from the request")
    })?;
    validate_host(&bare)
}

/// Whether a failed `gh auth token` says it doesn't know `--user` — the signature
/// of a pre-2.40 (pre-multi-account) binary. gh's command framework reports it as
/// `unknown flag: --user` on stderr and exits non-zero, with no code distinguishing
/// it from "this account has no token", so the wording is the only signal. Matched
/// on the long spelling this crate actually emits (never the `-u` shorthand), so a
/// different unknown flag can't be mistaken for it.
fn lacks_user_flag(stderr: &str) -> bool {
    let stderr = stderr.to_ascii_lowercase();
    stderr.contains("unknown flag") && stderr.contains("--user")
}

/// A one-line, bounded excerpt of a CLI's stderr for an error message: newlines
/// collapsed (so the whole hint survives a one-line `Display`) and capped, matching
/// processkit's own 200-byte diagnostic tail. Applied to **stderr only** — `gh`
/// prints the token on stdout, which no error here ever carries.
...
/// `GhAccountToken` — the per-account (`gh auth token --user`) credential
/// provider. Its own module: these tests drive the provider directly (and once
/// through the client), rather than the `GitHubApi` surface the others exercise.
#[cfg(test)]
mod account_token_tests {
    use super::*;
    use processkit::testing::{RecordingRunner, Reply, ScriptedRunner};

    /// A token whose bytes are distinctive enough that any leak into argv, a
    /// `Debug` rendering, or an error message is unambiguous.
    const ACCOUNT_TOKEN: &str = "gho_account_secret_do_not_leak";

    /// The `gh auth token` reply for a login that has no stored token, verbatim in
    /// the shape `gh` produces (`pkg/cmd/auth/token`: "no oauth token found for
    /// <host> account <login>", exit 1).
    fn no_oauth_token(host: &str, login: &str) -> Reply {
        Reply::fail(
            1,
            format!("no oauth token found for {host} account {login}"),
        )
    }

    /// The argv `gh auth token` is asked for carries the login — and the host when
    /// the request knows one — and nothing else. The token is never an argument
    /// (gh prints it on stdout), so command diagnostics can't observe it.
    #[tokio::test]
    async fn account_token_argv_names_the_login_and_known_host() {
        let rec = RecordingRunner::replying(Reply::ok(format!("{ACCOUNT_TOKEN}\n")));
        let provider = GhAccountToken::with_runner(&rec, "octocat");

        let request = CredentialRequest::new(CredentialService::GitHub).with_host("github.com");
        let cred = provider
            .credential(&request)
            .await
            .unwrap()
            .expect("an explicit login resolves a credential");
        assert_eq!(cred.secret().expose(), ACCOUNT_TOKEN);
        assert_eq!(cred.username(), None, "a bare token credential");
        ...
        assert!(
            !call.args_str().iter().any(|a| a.contains(ACCOUNT_TOKEN)),
            "the token must never reach argv: {:?}",
            call.args_str()
        );
    }

    // Without a host on the request (an unbound client), `--hostname` is omitted
    // entirely rather than guessed — gh then uses its own default host.
    #[tokio::test]
    async fn account_token_omits_hostname_when_the_request_has_none() {
        let rec = RecordingRunner::replying(Reply::ok(format!("{ACCOUNT_TOKEN}\n")));
        let provider = GhAccountToken::with_runner(&rec, "octocat");
        provider
            .credential(&CredentialRequest::new(CredentialService::GitHub))
            .await
            .unwrap()
            .expect("resolves without a host");
        assert_eq!(
            rec.only_call().args_str(),
    ...
    // The probe must not see an ambient token in its environment: `gh` would then
    // be able to answer with THAT token instead of the requested account's — a
    // silent identity swap. All four token vars gh reads are removed, and the probe
    // carries no injected credential of its own.
    #[tokio::test]
    async fn account_token_probe_runs_without_ambient_token_env() {
        let rec = RecordingRunner::replying(Reply::ok(format!("{ACCOUNT_TOKEN}\n")));
        let provider = GhAccountToken::with_runner(&rec, "octocat");
        provider
        ...
        for var in AMBIENT_TOKEN_VARS {
            assert_eq!(
                call.env(var),
                Some(None),
                "{var} must be REMOVED from the probe's environment, not merely unset by us"
            );
            assert!(!call.has_env(var), "{var} must never be set on the probe");
        }
    }

    // A login that can't be resolved is an ERROR naming the login and host — never
    // `Ok(None)`, which would quietly run the operation as the *active* account.
    #[tokio::test]
    async fn unknown_login_errors_instead_of_falling_back_to_ambient_auth() {
        let rec = RecordingRunner::new(
            ScriptedRunner::new().fallback(no_oauth_token("github.com", "ghost")),
        );
        let provider = GhAccountToken::with_runner(&rec, "ghost");
        let err = provider
            .credential(&CredentialRequest::new(CredentialService::GitHub).with_host("github.com"))
            .await
            .expect_err("an unresolvable explicit login must fail closed");

        let text = err.to_string();
        assert!(text.contains("ghost"), "the login must be named: {text}");
        assert!(
            text.contains("github.com"),
            "the host must be named: {text}"
        );
        assert!(
            matches!(err.reason(), ErrorReason::Exit { .. }),
            "a failed `gh auth token` is an exit failure: {err:?}"
        );

        // The same, with a gh diagnostic that mentions neither the login nor the
        // host: the identity context must come from OUR message, not from whatever
        // the CLI happened to print — that is what makes the failure actionable.
        let silent = GhAccountToken::with_runner(
            ScriptedRunner::new().fallback(Reply::fail(1, "you are not logged into any accounts")),
            "ghost",
        );
        let err = silent
            .credential(
                &CredentialRequest::new(CredentialService::GitHub).with_host("ghe.example.com"),
            )
            .await
            .expect_err("still fails closed");
        let text = err.to_string();
        assert!(
            text.contains("ghost") && text.contains("ghe.example.com"),
            "the login and host must come from our own message: {text}"
        );
    }

    // A pre-2.40 `gh` (before multi-account support) rejects `--user` with cobra's
    // `unknown flag`. That must become a clear "upgrade gh" error naming the
    // required version, not a bare non-zero exit the caller has to decode.
    #[tokio::test]
    async fn pre_multi_account_gh_reports_the_required_version() {
        for stderr in [
            "unknown flag: --user\n\nUsage:\n  gh auth token [flags]\n",
            "Error: unknown flag: --user\nUsage:\n",
        ] {
            let provider = GhAccountToken::with_runner(
                ScriptedRunner::new().fallback(Reply::fail(1, stderr)),
                "octocat",
            );
            let err = provider
                .credential(&CredentialRequest::new(CredentialService::GitHub))
                .await
                .expect_err("an unsupported --user flag must fail");
            let text = err.to_string();
            assert!(
                text.contains("2.40"),
                "the required gh version must be named: {text}"
            );
            ...
            assert!(
                ...
                "an out-of-date binary is classified like `ensure_supported`: {err:?}"
            );
        }
    }

    // A `gh` that exits 0 but prints nothing (or something that isn't a single-line
    // token) yields an error too — an empty secret would be treated as "no
    // credential" downstream and silently defer to ambient auth.
    #[tokio::test]
    async fn blank_or_multi_line_output_is_not_a_token() {
        for stdout in ["", "   \n", "line one\nline two\n"] {
            let provider = GhAccountToken::with_runner(
                ScriptedRunner::new().fallback(Reply::ok(stdout)),
                "octocat",
            );
            let err = provider
                .credential(&CredentialRequest::new(CredentialService::GitHub))
                .await
                .expect_err("output that is not a token must fail closed");
            assert!(
                matches!(err.reason(), ErrorReason::Parse { .. }),
                "unusable gh output is a parse failure: {err:?}"
            );
            assert!(err.to_string().contains("octocat"));
        }
    }

    // The resolution is cached per (login, host): a second request for the same
    // pair reuses the token instead of spawning `gh` again, while a DIFFERENT host
    // resolves separately — one host's token is never reused for another.
    #[tokio::test]
    async fn resolution_is_cached_per_login_and_host() {
        let rec = RecordingRunner::replying(Reply::ok(format!("{ACCOUNT_TOKEN}\n")));
        let provider = GhAccountToken::with_runner(&rec, "octocat");
        let saas = CredentialRequest::new(CredentialService::GitHub).with_host("github.com");

        for _ in 0..3 {
            let cred = provider.credential(&saas).await.unwrap().unwrap();
            assert_eq!(cred.secret().expose(), ACCOUNT_TOKEN);
        }
        assert_eq!(
            rec.calls().len(),
            1,
            "repeat requests for the same (login, host) must not respawn gh"
        );

        // Another host is a different cache key — and a different `gh` call.
        provider
            .credential(
                &CredentialRequest::new(CredentialService::GitHub).with_host("ghe.example.com"),
            )
        ...
        let calls = rec.calls();
        assert_eq!(calls.len(), 2);
        assert!(calls[1].args_str().contains(&"ghe.example.com".to_string()));
    }

    // A failed resolution is NOT cached: the provider stays usable once the cause
    // (a logged-out account, a transient gh failure) is gone.
    #[tokio::test]
    async fn a_failed_resolution_is_retried_rather_than_cached() {
        let rec = RecordingRunner::new(ScriptedRunner::new().on_sequence(
            ["gh", "auth", "token"],
            [
                no_oauth_token("github.com", "octocat"),
                Reply::ok(format!("{ACCOUNT_TOKEN}\n")),
            ],
        ));
        let provider = GhAccountToken::with_runner(&rec, "octocat");
        let request = CredentialRequest::new(CredentialService::GitHub).with_host("github.com");

        provider.credential(&request).await.unwrap_err();
        let cred = provider
            .credential(&request)
            .await
            .unwrap()
        ...
        assert_eq!(cred.secret().expose(), ACCOUNT_TOKEN);
        assert_eq!(rec.calls().len(), 2, "the failure was retried, not cached");
    }

    // The secret must not be printable: not through the provider's own `Debug`
    // (before or after resolution), and not through the text of an error — where a
    // captured stdout would otherwise carry the very token gh printed.
    #[tokio::test]
    async fn the_token_never_reaches_debug_or_an_error_message() {
        let provider = GhAccountToken::with_runner(
            ScriptedRunner::new().fallback(Reply::ok(format!("{ACCOUNT_TOKEN}\n"))),
            "octocat",
        );
        let request = CredentialRequest::new(CredentialService::GitHub).with_host("github.com");
        assert!(!format!("{provider:?}").contains(ACCOUNT_TOKEN));
        provider.credential(&request).await.unwrap().unwrap();

        let rendered = format!("{provider:?}");
        assert!(
            !rendered.contains(ACCOUNT_TOKEN),
            "a resolved token must not surface in Debug: {rendered}"
        );
        assert!(
            rendered.contains("octocat") && rendered.contains("cached_tokens"),
            "Debug still identifies the account and cache state: {rendered}"
        );

        // A gh run that printed the token on stdout and then failed: the error must
        // report the failure without carrying that stdout.
        let failing = GhAccountToken::with_runner(
            ScriptedRunner::new().fallback(
                Reply::fail(1, "you are not logged into any accounts").with_stdout(ACCOUNT_TOKEN),
            ),
            "octocat",
        );
        let err = failing.credential(&request).await.unwrap_err();
        assert!(
            !format!("{err}").contains(ACCOUNT_TOKEN)
                && !format!("{err:?}").contains(ACCOUNT_TOKEN),
            "the error must not carry the captured token: {err:?}"
        );
        assert_eq!(err.stdout(), Some(""), "no captured stdout rides along");
    }

    // A malformed login or host is refused BEFORE anything spawns — a flag-like
    // login would otherwise be parsed by gh as an option.
    #[tokio::test]
    async fn malformed_login_or_host_is_rejected_before_spawning() {
        for login in ["", "   ", "-u", "oct\nocat", "octo cat"] {
            let rec = RecordingRunner::replying(Reply::ok(format!("{ACCOUNT_TOKEN}\n")));
            let err = GhAccountToken::with_runner(&rec, login)
                .credential(&CredentialRequest::new(CredentialService::GitHub))
                .await
                .unwrap_err();
            assert!(
                vcs_cli_support::is_invalid_input(&err),
                "{login:?} must be rejected as invalid input: {err:?}"
            );
            assert!(rec.calls().is_empty(), "{login:?} must not spawn gh");
        }

        let rec = RecordingRunner::replying(Reply::ok(format!("{ACCOUNT_TOKEN}\n")));
        let err = GhAccountToken::with_runner(&rec, "octocat")
            .credential(&CredentialRequest::new(CredentialService::GitHub).with_host("-evil"))
            .await
            .unwrap_err();
        assert!(vcs_cli_support::is_invalid_input(&err), "{err:?}");
        assert!(rec.calls().is_empty());
    }
    ...
    async fn only_github_and_git_requests_are_served() {
        ...
        for service in [CredentialService::GitLab, CredentialService::Gitea] {
            assert!(
                provider
                    .credential(&CredentialRequest::new(service))
                    .await
                    .unwrap()
                    .is_none(),
                "{service:?} must not receive a GitHub token"
            );
        }
        assert!(
            rec.calls().is_empty(),
    ...
    // var gh reads for the bound host, and never into argv. This is the whole point
    // of the provider — the standard `with_credentials` path, with the token coming
    // from `gh auth token --user` instead of a literal.
    #[tokio::test]
    async fn account_provider_feeds_the_client_credential_path() {
        // `Arc` the recorder: attaching the provider to a client coerces it to
        ...
        let probe = Arc::new(RecordingRunner::replying(Reply::ok(format!(
            "{ACCOUNT_TOKEN}\n"
        ))));
        let provider = Arc::new(GhAccountToken::with_runner(Arc::clone(&probe), "octocat"));
        ...
        let call = rec.only_call();
        assert!(call.env_is("GH_ENTERPRISE_TOKEN", ACCOUNT_TOKEN));
        assert!(!call.has_env("GH_TOKEN"));
        assert!(
            !call.args_str().iter().any(|a| a.contains(ACCOUNT_TOKEN)),
            "the account token must never reach argv"
        );
        // The bound host reached the provider, so the probe asked for that host.
        let probe_call = probe.only_call();
        assert_eq!(
            probe_call.args_str(),
            [
                "auth",
        ...
        // And the probe stayed outside the client's credential injection: it
        // carries none of the token env the client sets (injecting into the probe
        // would mean the provider re-entering itself).
        for var in AMBIENT_TOKEN_VARS {
            assert!(
                !probe_call.has_env(var),
                "{var} must not be injected into the provider's own probe"
            );
        }
    }
}
