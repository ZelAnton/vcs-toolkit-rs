...
| Crate | Version | Tier |
|---|---|---|
| `vcs-diff` | 0.7.0 | settling |
| `vcs-cli-support` | 0.7.0 | settling |
| `vcs-testkit` | 0.7.0 | stable-ish (dev-only) |
| `vcs-git` | 0.11.0 | maturing |
| `vcs-jj` | 0.11.0 | maturing |
| `vcs-github` | 0.11.0 | maturing |
| `vcs-core` | 0.9.0 | evolving |
| `vcs-gitlab` | 0.7.0 | evolving |
| `vcs-gitea` | 0.7.0 | evolving |
| `vcs-forge` | 0.7.0 | evolving |
| `vcs-watch` | 0.7.0 | evolving |
| `vcs-mcp` | 0.7.0 | evolving |
...
- **Trait object-safety & mockability.** Every public trait stays object-safe
  ...
  `&[String]`/`&[PathBuf]`/`Option<PathBuf>`, not `&[&str]`/`Option<&str>`), so
  `&dyn Api` and `async-trait` keep working. Filesystem **paths** in the path-carrying
  surface are `PathBuf`/`Option<PathBuf>` (not `String`) so a non-UTF-8 name is
  lossless — an owned, object-safe type that also round-trips into the mutating APIs. The **wrapper** traits (`GitApi`/`JjApi`/
